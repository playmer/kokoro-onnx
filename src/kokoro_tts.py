...
def extract_text_from_epub(epub_file):
    ...
    for item in book.get_items():
        if item.get_type() == ebooklib.ebooklib.ITEM_DOCUMENT:
...
def extract_chapters_from_epub(epub_file, debug=False, info_file=None):
    ...
    if debug:
        ...
        for value in book.items:
            if type(value) is epub.EpubHtml:
                print(f"  {value.id}; {value.file_name}")
    ...
    def process_toc_items(items, depth=0):
        ...
        for item in spine:
            ...
            if doc:
                ...
                if text_content:
                    ...
                    if debug:
                        print("Content")
                        print(f"{'  ' * depth}Added chapter: {chapter_name}")
        ...
        #     def __str__(self):
        #         return f'[{self.index}, {self.item}, {self.file_name}, {self.fragment_id}, {self.subchapters}]'
        ...
        #         new_chapter = Chapter()
        #         new_chapter.index = i
        #         new_chapter.item = item
        #         new_chapter.file_name = href_parts[0]
        #         new_chapter.fragment_id = href_parts[1] if len(href_parts) > 1 else None
        #         new_chapter.subchapters = []
        ...
        #     item_to_process = to_process[i]

        #     if (i+1) < len(to_process):
        #         next_to_process = to_process[i + 1]
        #     else:
        #         next_to_process = None

        #     for j in range(len(spine)):
        #         if spine[j][1] == item_to_process.file_name:
        #             print(f'These are the same: {spine[j][1]}; {item_to_process.file_name}')
        #             should_append = True
                
        #         if (not spine[j][2]) or (next_to_process is None) or ((next_to_process is not None) and (spine[j][1] == next_to_process.file_name)):
        #             #print(f'We end: {spine[j][2]}; {next_to_process} {spine[j][1]}; {next_to_process.file_name}')
        #             should_append = False
        #             break
                    
        #         if should_append:
        #             item_to_process.subchapters.append(spine[j][1])
    ...
    if not chapters:
        ...
        docs = sorted(
            book.get_items_of_type(ebooklib.ebooklib.ITEM_DOCUMENT),
    ...
    if chapters:
        ...
        if debug:
            print("\nAvailable documents:")
            for doc in book.get_items_of_type(ebooklib.ebooklib.ITEM_DOCUMENT):
...
def process_chunk_sequential(chunk: str, kokoro: Kokoro, voice: str, speed: float, lang: str, 
    ...
    try:
        ...
        if not debug:
            ...
            sys.stdout.write(f"\nDEBUG: Full stack trace message: \n{traceback_str}")
        sys.stdout.write("\n")
        sys.stdout.flush()
...
def convert_text_to_audio(input_file, output_file=None, voice=None, speed=1.0, lang="en-us", 
                         stream=False, split_output=None, format="wav", debug=False, info_file=None):
    global stop_spinner
    # Load Kokoro model
    try:
        kokoro = Kokoro("kokoro-v1.0.onnx", "voices-v1.0.bin")
    ...
    if stream:
        ...
        if split_output:
            ...
            for chapter_num, chapter in enumerate(chapters, 1):
                ...
                #if info_file:
                #    print(f"LOG {chapter_num}: {chapter['content'].replace("\r\n", ".\r\n").replace("\n", ".\n")}", file=info_file)
                #continue
...
def kokoro_tts(
    ...
    with open("test.log", "w", encoding="utf-8") as f:
        convert_text_to_audio(input_file, output_file, voice=voice, stream=stream, 
                            ...
                            format=format, debug=debug, info_file=f)
